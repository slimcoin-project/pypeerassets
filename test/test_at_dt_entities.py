...
with open("dummy_txes.json", "r") as dummyfile:
...
def test_signalling_transaction_from_json():
    tx = e.SignallingTransaction.from_json(cls, dummy_signalling, provider)
    assert type(tx) == e.SignallingTransaction

def test_locking_transaction_from_json():
    tx = e.LockingTransaction.from_json(cls, dummy_locking, provider)
    assert type(tx) == e.LockingTransaction

def test_donation_transaction_from_json():
    tx = e.DonationTransaction.from_json(cls, dummy_donation, provider)
    assert type(tx) == e.DonationTransaction

def test_voting_transaction_from_json():
    tx = e.VotingTransaction.from_json(cls, dummy_voting, provider)
    assert type(tx) == e.VotingTransaction

def test_proposal_transaction_from_json():
    tx = e.ProposalTransaction.from_json(cls, dummy_proposal, provider)
